...
Item.findBySubId = (subId, mac, result) => {
  sql.query(
    ...
    ON item.idItem = itemCategory.itemId AND category.idCategory = subCategory.categoryId 
      ${mac ? "left JOIN favorites as fs ON item.idItem = fs.itemId AND fs.macAddress = '" + mac + "'" : ''} 
